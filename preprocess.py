...
num_paras = 0
num_valid_paras = 0
num_valid_cantonese_paras = 0
num_valid_mixed_paras = 0
num_valid_neutral_paras = 0
num_valid_mandarin_paras = 0
...
def process_page(obj_str, f):
    global num_paras, num_valid_paras, num_valid_cantonese_paras, num_valid_mixed_paras, num_valid_neutral_paras, num_valid_mandarin_paras
        ...
        for item_datum in item_data:
            ...
            for para in paras:
                ...
                if is_valid_para(para):
                    num_valid_paras += 1
                    para_lang = judge(para)
                    if para_lang == "cantonese":
                        num_valid_cantonese_paras += 1
                    elif para_lang == "mixed":
                        num_valid_mixed_paras += 1
                    elif para_lang == "neutral":
                        num_valid_neutral_paras += 1
                    elif para_lang == "mandarin":
                        num_valid_mandarin_paras += 1
                    else:
                        raise f"impossible paragraph language {para_lang}"
                    print(f"{para}\t{para_lang}", file=f)

def process_file(src_file):
    dst_file = path.join('processed', src_file.split("/")[-1])
